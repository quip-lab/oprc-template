...
EAS preview and production builds use `EXPO_PUBLIC_API_URL` for the deployed
web/API origin.
...
GitHub Actions runs three parallel Blacksmith jobs: Husky quality checks,
database-backed tests (including migrations), and the production build. The
stable `all-green` job is intended for branch protection. CodeQL analyzes
JavaScript and TypeScript on pull requests and on a weekly schedule; enable
code scanning in the repository security settings if it is not already active.
